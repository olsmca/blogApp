package org.olsmca.blogapp.web.rest;
...
import org.olsmca.blogapp.domain.Authority;
import org.olsmca.blogapp.domain.Blog;
import org.olsmca.blogapp.domain.User;
import org.olsmca.blogapp.repository.BlogRepository;
import org.olsmca.blogapp.repository.UserRepository;
import org.olsmca.blogapp.security.AuthoritiesConstants;
import org.olsmca.blogapp.security.SecurityUtils;
import org.olsmca.blogapp.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link org.olsmca.blogapp.domain.Blog}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class BlogResource {
    ...
    public List<Blog> getAllBlogs() {
        ...
        log.debug(" USER security " + userLogin.isPresent() + userLogin.get());
        log.debug(" USER findBase " + user.isPresent() + user.get());
        log.debug(" USER Autori " + user.get().getAuthorities());
        log.debug(" Autori " + AuthoritiesConstants.ADMIN);

        if (
            user.isPresent() &&
            user.get().getAuthorities().stream().map(Authority::getName).filter(AuthoritiesConstants.ADMIN::equals).findFirst().isPresent()
        ) {
